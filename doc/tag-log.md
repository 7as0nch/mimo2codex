...
- **[new]** **One-click adapter (Trope CUA) detect / download+build / uninstall from the Admin Plugins page**: the `mimo-computer-use` card now has a dedicated **Desktop backend: Trope CUA** panel. It probes whether the adapter is actually installed (running the plugin's `--doctor` server-side), shows the resolved executable path or the missing prerequisites (git / .NET 10 SDK), and offers **Download & build** / **Reinstall** / **Uninstall** buttons. Since Trope CUA ships as source, install runs an automated `git clone` + local build and **streams the build log live** into the UI over SSE. The built binary is installed **inside mimo2codex's own storage** (`~/.mimo2codex/adapters/trope-cua-bin`) rather than the global `%LOCALAPPDATA%\Programs\TropeCUA`, and on success the plugin's `[mcp_servers.*.env]` block is auto-pointed at it (`backend=trope` + `MIMO_COMPUTER_USE_TROPE_CMD`). Uninstall runs Trope CUA's own `uninstall.ps1 -InstallDir …` for both the mimo2codex and any legacy global install (binary + PATH cleanup) and then strips `MIMO_COMPUTER_USE_TROPE_CMD` from `config.toml` — it reports success on removing the *managed* installs (and warns, rather than fails, if a separate manual build still exists at the configured path, which lives outside mimo2codex storage and isn't auto-deleted). Detection/install only count the binary in mimo2codex's dir, so a failed rebuild can never masquerade as success or re-pin a stale external path. Install/uninstall also **stop any running Trope CUA `serve` daemon first** (it runs as `dotnet trope-cua.dll serve` and otherwise locks the source/output dirs → EBUSY / "folder in use"), and the source fetch self-heals a stale/partial non-git leftover dir by wiping and re-cloning. The build **bypasses trope's own `build.ps1`** (its `dotnet publish … --self-contained:$sc` colon form is misparsed by the .NET 10 SDK as `RuntimeIdentifier "-Configuration"` → NETSDK1083 and always fails); instead we reuse only trope's SDK/RID resolution and run a clean `dotnet publish … --self-contained true` straight into the managed dir. The adapter is installed under mimo2codex's **(possibly migrated) data dir** (`<dataDir>/adapters/trope-cua-bin`), not a fixed home path. Install also **verifies the produced binary is self-contained** and warns when it is framework-dependent (the cause of the "needs .NET 10" launch error). New endpoints `GET/POST /admin/api/plugins/:id/adapter[/install|/uninstall]`; new plugin CLI flag `--uninstall-adapter`. (`src/admin/router.ts`, `src/codex/plugins.ts`, `plugins/mimo-computer-use/server/lib/installers.mjs`, `web/src/pages/plugins/Plugins.tsx`)
- **[opt]** **`computer_state` hides hidden helper windows by default (trope backend)**: the adapter now defaults `on_screen_only=true` when mapping `computer_state` → `list_windows`, so always-present hidden windows (e.g. WeChat's `WxTrayIconMessageWindow` tray-icon sink) no longer clutter the result. The model can still pass `on_screen_only=false` to see everything. (`plugins/mimo-computer-use/adapters/shared/trope/index.mjs`)
- **[new]** **`mimo-computer-use` local Codex plugin scaffold**: a new isolated plugin lives under `plugins/mimo-computer-use/` and exposes a conservative MCP computer-control surface (`computer_state`, `computer_click`, `computer_type`, `computer_key`, `computer_scroll`, `computer_wait`) for MiMo, DeepSeek, and other OpenAI-compatible function-calling models. It does **not** change the proxy's `computer_use_preview` handling or any existing runtime code. The plugin delegates real desktop automation to a single cross-platform adapter, [Trope CUA](https://github.com/voctory/trope-cua) (macOS / Windows / Linux), and returns text JSON summaries / screenshot paths instead of image tool results so MiMo/DeepSeek history stays compatible.
- **[new]** **Admin Plugins page + unified `MIMO2CODEX_PLUGIN` env control**: the admin console now has a dedicated **Plugins** route for built-in local Codex plugins, starting with `mimo-computer-use`. When `MIMO2CODEX_PLUGIN` is unset, the page can enable/disable the plugin and hot-write the `[mcp_servers.mimo-computer-use]` block into `~/.codex/config.toml`; when the env var is set (`mimo-computer-use` / `computer-use` / `1` / `on` to enable, `0` / `off` / `none` to disable), the env value wins and the UI switch is locked read-only for predictable deployments. The plugin directory is now included in npm package files, Docker images, and the desktop sidecar bundle so CLI, desktop, and container deployments all see the same built-in plugin inventory.
- **[new]** **First-use adapter detection and guided install for computer use**: `mimo-computer-use` now exposes `npm run doctor`, `npm run install-adapter`, and the MCP tool `computer_install_adapter`, all targeting the [Trope CUA](https://github.com/voctory/trope-cua) backend (macOS + Windows). Trope CUA ships as source, so the installer `git clone`s it into `~/.mimo2codex/adapters/trope-cua` and runs the platform build script (`install-macos.sh` / `install-windows.ps1 -SelfContained`); prerequisites are `git` plus Xcode Command Line Tools (macOS) or the .NET SDK (Windows). The missing-adapter diagnostics include the install docs, and the built-in `mimoskill/scripts/computer_use_setup.py` lets MiMo-backed workflows detect / install the adapter before attempting desktop control. `mimoskill/` is bundled for npm/source, desktop sidecar, and Docker image layouts.
- **[doc]** **Detailed plugin guide**: added `doc/mimo-computer-use-plugin.zh.md` with architecture, Admin UI/env enablement, first-use adapter install flow, mimoskill integration, CLI/desktop/Docker packaging notes, and troubleshooting.
- **[doc]** **Quick-start guide for enabling `mimo-computer-use` in an existing Codex setup**: the plugin docs now include the exact `~/.codex/config.toml` `[mcp_servers.mimo-computer-use]` snippet, restart step, `npm run doctor` diagnostic command, adapter install notes, and a first Codex prompt to verify `computer_state`.
- **[fix]** **`mimo-computer-use` MCP server now speaks the MCP stdio wire format (newline-delimited JSON)**: the plugin server framed its responses with LSP-style `Content-Length` headers, which the MCP stdio transport used by Codex (and by the Python/TS adapters it shells out to) does not understand — Codex never received the `initialize` response and dropped the server after `startup_timeout_sec`, so the plugin was effectively unusable. `writeMessage` now emits newline-delimited JSON; `MessageReader` still accepts legacy `Content-Length` frames for robustness. (`plugins/mimo-computer-use/server/lib/framing.mjs`)
- **[fix]** **External adapter launch failures no longer crash the plugin**: `callExternalMcp` (the Trope CUA backend) had no `child.on("error")` handler, so a failed `spawn` (ENOENT / EACCES — e.g. the command vanished after the existence check) threw an uncaught exception that killed the whole `mimo-computer-use` server instead of returning an error. It now degrades to a structured `{ ok: false }` result immediately, without crashing or waiting out the 30s request timeout. (`plugins/mimo-computer-use/server/lib/external-mcp-client.mjs`)
