...
- **[new]** **Computer Use plugin rewritten in pure Node (nut.js) — Trope CUA removed, no compiler toolchain**: `mimo-computer-use` used to forward to the external **Trope CUA** binary, which had to be `git clone`d and built with **Xcode Command Line Tools (macOS) / .NET 10 SDK (Windows)** — slow, heavy, fragile. It now drives the desktop directly with **nut.js** (prebuilt native bindings — a plain `npm install`, no compiler). The six `computer_*` tools (state/click/type/key/scroll/wait) are unchanged, so any function-calling model routed through mimo2codex (MiMo, DeepSeek, OpenAI-compatible) can use them. Perception is **A+C**: `computer_state` returns the screenshot as an MCP image for vision models, and with `ocr:true` also returns OCR'd text **targets with click coordinates** (via `mimoskill/scripts/ocr.py --boxes`, tesseract — local/offline) so text-only models can click by label. The real cursor glides visibly to each target before clicking.
- **[new]** **Glowing on-desktop cursor (optional Electron overlay) + live "Computer Use Monitor" page**: from admin → Plugins you can download Electron to show a glowing cyan cursor + click ripples on the **real desktop** while the AI works, with a draggable "Stop / Resume AI control" pill and a global hotkey (`Ctrl/Cmd+Alt+Esc`) that immediately halts further actions. A new admin **Computer Use Monitor** tab streams the AI's actions (screenshot + virtual cursor trail + action log) live over SSE — and works even without Electron installed.
- **[new]** **Human-takeover watch — auto-pause when you grab the mouse**: the agent and the user share one physical cursor, so the plugin now records where the agent left the pointer and, before each action, reads the real cursor position; if you've moved it (beyond a small threshold), the action is **paused** with a `user_intervened` result that tells the model to stop and ask what to change, and the Monitor page shows a "you took over — paused" banner. Re-observing with `computer_state` re-baselines and lets it continue. Pure nut.js (`mouse.getPosition`, no Electron needed). Toggle with `MIMO_COMPUTER_USE_WATCH=off`; tune the pixel threshold with `MIMO_COMPUTER_USE_WATCH_THRESHOLD` (default 8).
- **[opt]** **Removed all Trope CUA code/config/docs**: dropped the external-MCP forwarder, the clone/.NET/Xcode installer + prerequisite/uninstall machinery, and the `MIMO_COMPUTER_USE_BACKEND` / `MIMO_COMPUTER_USE_TROPE_*` env knobs. The admin "download" button now runs `npm install` (nut.js required; Electron optional via a checkbox) and no longer pins an executable path into `config.toml`; the MCP block instead carries `MIMO2CODEX_ADMIN_URL` so the plugin can stream actions to the Monitor page.
- **[fix]** **Vision models can finally SEE computer-use screenshots; screenshots no longer break the session**: two problems compounded. (a) `computer_state` embedded a full ~1MB base64 screenshot every call, growing the history Codex resends each turn until the upstream request died mid-stream ("stream disconnected before completion" — a *new* chat worked, the bloated one didn't). (b) Even after switching to a vision model, the model still couldn't see anything: Chat Completions **`tool` messages can't carry images**, so the screenshot embedded in a tool result was always stripped before reaching MiMo. Fixes: the plugin now embeds a **hard-downscaled** JPEG (`jimp`, ≈70KB at 1024px/q50, tunable via `image_width`/`image_quality`) so it can't bloat the session; and `reqToChat`, for **vision-capable** providers, **relocates the tool-output image into a following `user` message** (which can carry images) so e.g. `mimo-v2.5` actually sees the screen. Non-vision models keep the strip-with-OCR-hint behavior, and OCR text `targets` are returned by default (`ocr:true`) so text-only models can click by label; when neither an image nor targets are available the result carries a `hint` telling the model NOT to guess coordinates.
- **[fix]** **Computer-use Monitor screenshot + Electron download (China network)**: (1) the Monitor "screen" image 404'd whenever the proxy's data dir differed from where the Codex-launched MCP server wrote frames (the MCP env only carried `MIMO2CODEX_ADMIN_URL`, so frames went to the default `~/.mimo2codex/computer-use/frames`) — the `/admin/api/computer-use/frame/:name` endpoint now checks **both** the data-dir location and the home-dir default. (2) `npm install electron` exits 0 in mainland China while the ~150MB binary never downloads (GitHub blocked → no `dist/`/`path.txt` → overlay shows "未安装"); the install flow now defaults `ELECTRON_MIRROR` to the npmmirror Electron mirror and **force-fetches the binary via `electron/install.js`** when it's still missing, and reports a clear failure message otherwise.
- **[opt]** **Admin Plugins page redesigned as a card marketplace + detail drawer; docweb gets a Plugins page**: the admin Plugins tab now shows a compact card grid (click a card → a right-side detail drawer with the enable toggle, config, dependency/Electron install, and a Monitor shortcut) instead of one big inline card, plus a dashed **"Contribute a plugin"** card linking to the docweb idea-submission page. docweb adds a new **Plugins** route (`/plugins`, in the top nav) introducing the built-in `mimo-computer-use` plugin with a version badge (**v0.2.0**) and a "submit your adapter" CTA → `/ideas/new`.
