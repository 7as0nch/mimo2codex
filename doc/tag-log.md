...
- **[new]** **First-use adapter detection and guided install for computer use**: `mimo-computer-use` now exposes `npm run doctor`, `npm run install-adapter`, and the MCP tool `computer_install_adapter`. On macOS the installer uses `brew install steipete/tap/peekaboo`; on Windows it uses `uv tool install windows-mcp` or falls back to `uvx windows-mcp`. The missing-adapter diagnostics now include install commands, and the built-in `mimoskill/scripts/computer_use_setup.py` lets MiMo-backed workflows detect / install the adapter before attempting desktop control. `mimoskill/` is bundled for npm/source, desktop sidecar, and Docker image layouts.
