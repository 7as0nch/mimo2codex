...
- **[new]** **Computer Use 插件改为纯 Node（nut.js）—— 移除 Trope CUA，免编译工具链**：`mimo-computer-use` 此前把工具转发给外部 **Trope CUA** 二进制，安装要 `git clone` 再用 **Xcode 命令行工具（macOS）/ .NET 10 SDK（Windows）** 编译，链路长、依赖重、易碎。现在直接用 **nut.js**（自带预编译原生插件，`npm install` 即可，无需编译器）操作桌面。6 个 `computer_*` 工具（state/click/type/key/scroll/wait）保持不变，所以任何经 mimo2codex 走 Responses 协议、会函数调用的模型（MiMo、DeepSeek、OpenAI 兼容）都能用。感知为 **A+C**：`computer_state` 给视觉模型回传 MCP 截图图像，`ocr:true` 时还回传 **带点击坐标的 OCR 文字目标**（经 `mimoskill/scripts/ocr.py --boxes`，tesseract，本地离线），让纯文本模型也能「按文字点击」。点击前真实光标会平滑滑到目标位置。
- **[new]** **真实桌面发光光标（可选 Electron overlay）+ 实时「Computer Use 监看」页**：在 admin → 插件页可下载 Electron，AI 操作时在**真实桌面**显示发光青色光标 + 点击波纹，并提供可拖动的「停止 / 恢复 AI 控制」按钮和全局热键（`Ctrl/Cmd+Alt+Esc`，按下即刻阻止后续操作）。新增 admin **Computer Use 监看** 标签页，通过 SSE 实时显示 AI 的操作（截图 + 虚拟光标轨迹 + 操作日志）——**不装 Electron 也能用**。
- **[new]** **人工接管监测 —— 你一动鼠标就自动暂停**：AI 和你共用一只物理光标，插件现在会记录它把光标留在哪，每次动作前读一下真实光标位置；若你移动了（超过阈值），该动作就**暂停**并返回 `user_intervened`，让模型停手、问你要怎么改，监看页弹出「你已接管，已暂停」横幅。AI 重新 `computer_state` 观察后会重新基准并继续。纯 nut.js（`mouse.getPosition`，不需要 Electron）。`MIMO_COMPUTER_USE_WATCH=off` 关闭；`MIMO_COMPUTER_USE_WATCH_THRESHOLD`（默认 8 像素）调阈值。
- **[opt]** **删除全部 Trope CUA 代码/配置/文档**：移除了外部 MCP 转发器、clone/.NET/Xcode 安装器与先决条件/卸载逻辑，以及 `MIMO_COMPUTER_USE_BACKEND` / `MIMO_COMPUTER_USE_TROPE_*` 环境变量。admin「下载」按钮现在执行 `npm install`（nut.js 必装；Electron 由勾选可选），且不再往 `config.toml` 写入可执行文件路径；MCP 块改为携带 `MIMO2CODEX_ADMIN_URL`，让插件能把操作推送到监看页。
- **[fix]** **视觉模型终于能“看见”computer-use 截图；截图也不再撑爆会话**：两个问题叠加。(a) `computer_state` 每次都把 ~1MB base64 截图塞进结果，Codex 每轮重发的历史越滚越大，最终上游请求中途断流（“stream disconnected before completion”，表现为*新*会话正常、旧会话崩）。(b) 就算换了视觉模型也看不到：Chat Completions 的 **`tool` 消息根本不能带图**，嵌在工具结果里的截图到 MiMo 前一定被剥掉。修复：插件改为嵌入**强力降采样**的 JPEG（`jimp`，1024px/q50 约 70KB，可用 `image_width`/`image_quality` 调），不再撑爆会话；并且 `reqToChat` 对**视觉模型**会把**工具结果里的图片搬到紧随其后的 `user` 消息**（user 消息可以带图），这样 `mimo-v2.5` 就真能看到屏幕。非视觉模型仍走“剥图 + OCR 提示”，OCR 文字 `targets` 默认返回（`ocr:true`）让纯文本模型按文字点击；当既无图又无目标时返回 `hint` 提示模型**别瞎猜坐标**。
- **[fix]** **Computer-use 监看截图 + Electron 下载（国内网络）**：(1) 当代理的 data 目录与 Codex 拉起的 MCP server 写帧目录不一致时，监看页「屏幕画面」会 404（MCP env 只带了 `MIMO2CODEX_ADMIN_URL`，帧落到了默认的 `~/.mimo2codex/computer-use/frames`）——`/admin/api/computer-use/frame/:name` 现在**同时在 data 目录与 home 默认目录两处查找**。(2) 国内 `npm install electron` 会 exit 0 但那 ~150MB 二进制根本没下下来（GitHub 被墙 → 没有 `dist/`/`path.txt` → 显示「未安装」）；安装流程现在把 `ELECTRON_MIRROR` 默认指向 npmmirror 镜像，并在二进制仍缺失时**用 `electron/install.js` 强制重下**，失败也给出清晰提示。
- **[opt]** **admin 插件页改成卡片市场 + 详情抽屉；docweb 新增插件页**：管理台插件页从「一张大卡片平铺所有信息」改为**紧凑卡片网格**（点卡片右侧滑出详情抽屉：启用开关、配置、依赖/Electron 安装、监看页快捷入口），并多了一张虚线 **「欢迎投稿插件适配」** 卡，点击跳转 docweb 投稿页。docweb 新增 **插件** 路由（`/plugins`，已加进顶部导航），介绍自带的 `mimo-computer-use` 插件并带**版本号徽标 v0.2.0**，以及「投稿你的适配」CTA → `/ideas/new`。
